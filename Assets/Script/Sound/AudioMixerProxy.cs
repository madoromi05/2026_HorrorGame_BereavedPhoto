...
/// AudioMixer 側には "BgmVolume" と "SeVolume" の Exposed Parameter が必要。
/// </summary>
public class AudioMixerProxy : MonoBehaviour
{
    ...
    public void MuteBgm(bool mute) => _mixer?.SetFloat(BgmParam, mute ? -80f : ToDb(_bgmVolume));
    public void MuteSe(bool mute)  => _mixer?.SetFloat(SeParam,  mute ? -80f : ToDb(_seVolume));
